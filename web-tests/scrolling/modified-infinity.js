...
!function(window, Math, $) {
  ...
  function insertPagesInView(listView) {
    dump(`listView insertPagesInView\n`);
  ...
  function updateStartIndex(listView) {
    ...
    dump(`listView scroll stashing old pages\n`);
  ...
  function repartition(listView) {
    ...
    for(index = 0, length = pages.length; index < length; index++) {
      dump(`listView repartition visit page\n`);
      ...
      dump(`listView repartition remove page\n`);
      currPage.remove();
    }
